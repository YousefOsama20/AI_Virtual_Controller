...
`autopy` requires C++ build tools on Windows:

1. Install [Microsoft C++ Build Tools](https://visualstudio.microsoft.com/visual-cpp-build-tools/).
2. Then run: `pip install autopy`

### `IndexError: list index out of range`

This typically occurs when hands are not fully detected. The code includes safety checks, but if you encounter this:

- Ensure good lighting and a clear background.
- Keep both hands fully visible in the camera frame.
- Adjust `detectionCon` and `trackCon` in `library.py` (try lowering to `0.5`).
...
### `PermissionError` When Taking Screenshots
...
- Ensure `pycaw` and `comtypes` are installed.
- This feature only works on **Windows** with a standard audio endpoint.
...
- Update this README if your changes add new features or alter behavior.
- Report bugs via [GitHub Issues](https://github.com/YousefOsama20/AI_Virtual_Controller/issues).
